...
###        sigma - scalar, regression standard error from step 1
    ...
    if( any( is.na( sigma ) ) ){
      ...
      return( exp( -( res )**2 / ( 2 * sigma**2 ) ) / ( sqrt( 2 * pi ) * sigma ) )
    }
}
...
compute_gps_from_kde = function( ols ){
### input: ols object from lm  
  # use max lik estimator for the regression standard error, sigma
  # require( ks )
  res    = ols$residuals
  fhat   = kde( x = res)
  gps_kde = predict( fhat, x = fhat$x )
  sigma  = sqrt( deviance( ols )/( df.residual( ols ) + ols$qr$rank ) )
  
  gps_norm = dnorm( res, mean = 0, sd = sigma )
  
  df.gps = data.frame( res = res, gps_kde = gps_kde, gps_norm = gps_norm )
  
  list( ols_dose_res = res, gps_kde = gps_kde, gps_norm = gps_norm, sigma = sigma, df.gps = df.gps )
}
...
func_check_covariate_balance_agps = function( opt, db_coba, sVars2test ){ # db_coba = db_dose; sVars2test = opt$ols_dose_coba 
      ...
      coba_agps_sum = lapply( seq(1:opt$dose_group_num), function( dose_group_k ) { # dose_group_k = 1
          ...
          db_coba_k = db_coba %>%  mutate( med_dose_group_k = med_dose_group_k ) %>%
                                   mutate( res = med_dose_group_k - fit )
          
          ### compute GPS at med_dose_group_k for all entries
          if( opt$compute_gps_from_ols){
            
            db_coba_k %<>% mutate( gps_at_med_dose_group_k = compute_gps( med_dose_group_k, fit, sigma ) )
            
         }else{
           
            kde_hat_ols   = kde( x = opt$object_ols_dose$residuals )
            db_coba_k %<>% mutate( gps_at_med_dose_group_k = predict( kde_hat_ols, x = db_coba_k$res ) )
      ...
      ### collect GPS evaluated at the representative point of each treatment interval NB!!! SOMETHING IS WRONG HERE
      gps_at_med_dose_group_k = do.call( cbind, lapply( coba_agps_sum, `[[`,  "gps_at_med_dose_group_k" ) )
      
      gps_at_med_dose_group_k %>% head
...
# svar_x_list = sVars2test; svar_dose = opt$varname_dose_orig; db = db_agps; scale_dose = 100
  ...
  if( opt$compute_gps_from_ols ){
    ...
    db %<>% mutate( gps_dose_q25 = compute_gps( dose_q25, fit, sigma ),
                    gps_dose_q50 = compute_gps( dose_q50, fit, sigma ),
                    gps_dose_q75 = compute_gps( dose_q75, fit, sigma ) )    
    
  }else{
    db %<>% mutate( res_q25 = dose_q25 - fit,
                    res_q50 = dose_q50 - fit,
                    res_q75 = dose_q75 - fit )  
    kde_hat_ols   = kde( x = opt$object_ols_dose$residuals )
    db %<>% mutate( gps_dose_q25 = predict( kde_hat_ols, x = db$res_q25 ),
                    gps_dose_q50 = predict( kde_hat_ols, x = db$res_q50 ),
                    gps_dose_q75 = predict( kde_hat_ols, x = db$res_q75 ) )
...
func_impose_common_support = function( opt, db_cosu, doPlotGPS ){ # db_cosu = db_dose; doPlotGPS = TRUE
### doPlotGPS - option for extracting gps for making historgram plots  
    ...
    cosu_by_group = lapply( seq( 1:opt$dose_group_num ), function( dose_group_k ){ # dose_group_k = 1
        ...
        if( opt$compute_gps_from_ols ){
            ...
            db_cosu %<>% mutate( gps_dose_med_k = compute_gps( dose_med_k, fit, sigma ) )          
          
        }else{
          
            kde_hat_ols   = kde( x = opt$object_ols_dose$residuals )
            db_cosu %<>% mutate( res = dose_med_k - fit ) 
            db_cosu %<>% mutate( gps_dose_med_k = predict( kde_hat_ols, x = db_cosu$res ) )
...
func_compute_adrf          = function( opt, ols4predict ){ # ols4predict = ols_resp
### in: ols4predict - lm object
  adrf = lapply( opt$dose_levels_adrf, function( dose_tau ){ #} dose_tau = opt$dose_levels_adrf[4]; ols4predict = ols_resp
    
    ### apply log-transform for dose if necessary
    newdata_dose = ifelse( opt$ols_dose_log, log( dose_tau ), dose_tau )
    
    if( opt$compute_gps_from_ols ){
        gps = compute_gps( newdata_dose, db_dose$fit, db_dose$sigma)
    }else{
        kde_hat_ols   = kde( x = opt$object_ols_dose$residuals )
        gps = predict( kde_hat_ols, x = newdata_dose - db_dose$fit )
    }
      
    newdata = data.frame( dose_tau, gps = gps, incpt = 1  ) %>%
...
opt$compute_gps_from_ols = FALSE # TRUE # 
...
opt$ols_dose          = c( "capex_Ln","capex_Ln2",
                           ...
                           "country","sector","country*tech") #,"tech""country","sector") # "capex_Ln*tech","mcap_Ln*tech",
...
# opt$ols_resp          = c( "log(rdi)","I(log(rdi)^2)","I(log(rdi)^3)","gps" )
# opt$ols_resp          = c( "log(rdi)","I(log(rdi)^2)","I(log(rdi)^3)","I(log(gps))" )
# opt$ols_resp          = c( "log(rdi)","I(log(rdi)^2)","I(log(rdi)^3)","I(log(gps))","I(log(gps)*log(rdi))" )
#opt$ols_resp          = c( "log(rdi)","I(log(rdi)^2)","I(log(rdi)^3)","I(log(gps))","I(log(gps)^2)" )
# opt$ols_resp          = c( "log(rdi)","I(log(rdi)^2)","I(log(rdi)^3)","gps","I(gps * log(rdi))","I(gps * log(rdi)^2)" )
# opt$ols_resp          = c( "log(rdi)","I(log(rdi)^2)","I(log(rdi)^3)","gps","I(gps * log(rdi))","I(gps * log(rdi)^2)","I(gps * log(rdi)^3)" )
# opt$ols_resp          = c( "log(rdi)","I(log(rdi)^2)","I(log(rdi)^3)","I(log(rdi)^4)","gps","I(gps * log(rdi))" )
# opt$ols_resp          = c( "log(rdi)","I(log(rdi)^2)","gps","I(gps * log(rdi))" )
...
if( opt$compute_gps_from_ols ){
  ...
  ud_ols_dose = compute_gps_from_kde( ols_dose )
  db_dose %<>% mutate( gps = ud_ols_dose$gps_kde, fit = ols_dose$fitted.values, sigma = NA )
}
...
doseresponse_boot_list = lapply( ols_dose_boot$boot.list, function( boot ){ #boot = ols_dose_boot$boot.list[[2]]
    ...
    boot_sigma  = ifelse( opt$compute_gps_from_ols, boot$sigma, NA ) 
    ...
    if( opt$compute_gps_from_ols ){
        ...
        boot_res = boot_actual - boot_fitted
        fhat     = kde( x = boot_res)
        boot_gps = predict( fhat, x = fhat$x )
    }
    ...
    if( opt$impose_common_support ){
     ud_cosu_boot = func_impose_common_support( opt, db_resp_boot, FALSE ) ### HERE IS WRONG 
...
if( opt$compute_gps_from_ols ){
  ...
  ggplot( ud_ols_dose$df.gps ) + 
    geom_histogram( aes( x=res,y=..density..), position="identity") + 
    geom_density(   aes( x=res,y=..density..), color = "blue") +
    geom_line(      aes( x=res, y= gps_kde  ), color = 'red' )+
    geom_line(      aes( x=res, y= gps_norm ), color = 'yellow' )
}
@   
